		...
		<div id="rolex-search" class="section-inner">
		
		<script>
		
			$(function () {
				
			$('.search-shop-list').css('height','auto');
		...
		<section class="search-form">
			...
			<div class="section-inner">
				<div class="search-shop" data-uniq-id="609bb70d69164">
					<div class="title color-white lts0 ta-c search-rform-title">お近くのジュエルカフェを<br class="sp">今すぐチェック！</div>

					<div class="api_current_location_button"><a href="<?php echo esc_url(home_url('/shop_map/')); ?>" target="_blank">現在地から探す</a></div>

					<div class="title color-white lts0 ta-c search-rform-title">住所・店舗名から探す</div>
...
<div class="container -cushion">
	...
	<div class="d-f ai-c">
		<input type="text" id="skeyword" name="s" value="" placeholder="例：横浜市" maxlength="40" autocomplete="off">

		<input type="button" class="search-rolex-btn" id="form_search_btn" value="検索" />
							...
							<?php if($the_query->have_posts()):?>
								<?php while ($the_query->have_posts()): ?>
									...
									<?php
										...
										if(isset($exclude_store[0]) && $exclude_store[0]== '1'){
									...
									?>
									<a href="<?php echo $kaitori_type_url; ?>/shop/<?php echo $pt->slug . '/'; if (strpos($_SERVER['REQUEST_URI'], '/blog/') !== 0) { 
...
<div class="shop-result2 mt-10">
	...
	<div class="ttl-box-red" style="font-weight:bold;">検索結果 <span class="yellow">0</span>件</div>
...
$('#form_search_btn').on('click', function() {
	...
	if(isPostcode(hankaku2Zenkaku($("#skeyword").val()))){
		
		var zipcode = hankaku2Zenkaku($("#skeyword").val());
	  ...
	  $.getJSON('https://zipcloud.ibsnet.co.jp/api/search?callback=?',
			...
			$.ajax({
				...
				success: function(response){
					...
					if( response.length  < 1){
						...
						$("#shop-res2").append('<li class="shop-result-item bg-search marker-yellow"><div class="shop-name bold">該当する店舗住所が存在しません</div></li>');
		...
		var scroll = $('#rolex-search').offset().top + 380;
	
		$("html, body").animate({ scrollTop: scroll }, 1000);

	}else{
			
			var address = zenkana2Hankana($("#skeyword").val());
			...
			$.ajax({
				...
				success: function(response){
					...
					$('.shop-result2 .yellow').html(Object.keys(response).length);
					...
					$.each(response,function(key,obj){
						...
						if (strpos($_SERVER['REQUEST_URI'], '/kaitori/gold/') !== false) {
						?>
							obj.url = '/kaitori/gold' + obj.url;
						<?php }else if (strpos($_SERVER['REQUEST_URI'], '/kaitori/diamond/') !== false) { ?>
							obj.url = '/kaitori/diamond' + obj.url;
						<?php }else if (strpos($_SERVER['REQUEST_URI'], '/kaitori/tokei/rolex-top/') !== false) { ?>
							obj.url = '/kaitori/tokei/rolex-top' + obj.url;
						<?php }else if (strpos($_SERVER['REQUEST_URI'], '/kaitori/letter-top/') !== false) { ?>
							obj.url = '/kaitori/tokei/letter-top' + obj.url;
						<?php }else if (strpos($_SERVER['REQUEST_URI'], '/kaitori/jewelry/') !== false) { ?>
							obj.url = '/kaitori/jewelry' + obj.url;
						<?php }else if (strpos($_SERVER['REQUEST_URI'], '/kaitori/brand/vuitton/') !== false) { ?>
							obj.url = '/kaitori/brand/vuitton' + obj.url;
						<?php
						}
						?>
				...
				success: function(response){
					...
					$.each(response,function(key,obj){
						...
						if(obj.tel && obj.time){
							...
							$("#shop-res2").append('<li class="shop-result-item bg-search marker-yellow"><div class="shop-result-name"><div class="shop-name bold"><a href="'+obj.url+'"  class="">ジュエルカフェ '+obj.title+'</a></div><div class="shop-address d-f">'+obj.add+'</div></div><div class="shop-result-tel"><a class="shop-tel bold color-red" href="tel:'+obj.tel+'">TEL '+obj.tel+'</a></div></li>');
							
						}else if(obj.time){

							$("#shop-res2").append('<li class="shop-result-item bg-search marker-yellow"><div class="shop-result-name"><div class="shop-name bold"><a href="'+obj.url+'"  class="">ジュエルカフェ '+obj.title+'</a></div><div class="shop-address d-f">'+obj.add+'</div></div><div class="shop-result-tel"><div class="shop-opening">受付時間 '+obj.time+'</div></div></li>');
	
						}else{
							
							$("#shop-res2").append('<li class="shop-result-item bg-search marker-yellow"><div class="shop-result-name"><div class="shop-name bold"><a href="'+obj.url+'"  class="">ジュエルカフェ '+obj.title+'</a></div><div class="shop-address d-f">'+obj.add+'</div></div><div class="shop-result-tel"></div></li>');
