<?php
	...
	$sql = "SELECT count(*) FROM `wp_posts` where post_parent = {$post_id}  and post_status = 'publish' and post_type = 'kaitori' " ;
	...
	if($count < 1 && trim(get_field('ライン1' , $post_id)) == '' ){
...
.brand-pricetable .accordion .accordion-model i{
	...
	left:8px;
	top:50%;
	display:inline-block;height:6px;transform:translateY(-50%);
	transform:rotate(-45deg);
	transition:.2s;width:6px;
}
...
.accordion-content2{background:#f1f1f1;padding:10px;display:none;}
...
@media (min-width: 800px) {
	...
	.accordion-content2 td:first-child{width:45%;}
