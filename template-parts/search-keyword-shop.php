	...
	.kaitori .search-keyword-shop{background:#de1022;border-radius:10px;padding:20px;}


	.search-title{font-size:17px;color:#fff;font-weight:bold; line-height: 1.7;}
	.search-title span{color:#ffef00;border-bottom:2px solid #ffef00;}


	.search-label{color:#de1022;}
	.search-label span{font-size:1rem;font-weight:bold;}
	.search-label p{font-size:11px;font-weight:bold;}


	.search-form-wrap{display:flex;margin-top:20px;margin-bottom:10px;}
	...
	.search-form input[type="text"]{
		width: 100%;
		border:0px;
		border-bottom:2px solid #de1022;
		padding:7px 0;
	...
	.search-btn{position:relative;width:74px;background:#000;border-top-right-radius:10px;border-bottom-right-radius:10px;}
	.search-btn input[type="image"]{position:absolute;top:50%;left:50%;    transform: translate(-50%, -50%); -webkit-transform: translate(-50%, -50%); -ms-transform: translate(-50%, -50%);background:#000;color:#fff;border:0px;width:35px;height:35px;}


	.search-keyword-shop .search-shop{background:transparent;margin-bottom:0px;width:100%;}
	.search-keyword-shop .search-shop .border{width:60%;}
	...
	@media (min-width: 1000px) {
		...
		.search-label span{font-size:1.5rem;font-weight:bold;}
		...
		.search-form input[type="text"]{font-size:1.2rem;padding: 7px;width: calc( 100% - 330px ) ; }

	}
	
	
	
	.audio-search{position:absolute;bottom:10px;right:0px;}
	
	@media (min-width: 1000px) {
	
		.audio-search{position:absolute;top:5px;right:0px;}
	
	}
	
	
	
	.audio-search:hover{cursor:pointer;}
	
	.audio-search img{height:55px;}
...
<style>
	.search-shops div[data-shop-wrap='東京都'] a:first-of-type{
		display:none!important;
	}
</style>


<section class="search-keyword-shop">


<div class="search-keyword">

	<div class="search-title ta-c">
		<p>全国<?php echo get_option('shop'); ?>店舗のジュエルカフェで</p>
		<p><span>最短15分スピード査定</span>・<br class="only-sp"><span>即日現金買取</span>を行っています！</p>
	</div>
	...
	<div class="search-form-wrap">
		...
		<div class="search-form">
			...
			<label class="search-label">
				<span>フリーワード検索店舗検索</span>
			</label>

			<input type="text" id="skeyword" name="s" value="" placeholder="キーワードもしくは郵便番号を入力してください。" maxlength="40" />
			
		
		<!--
		<div class="audio-search" onclick="recognition.start()">
			<img src="https://jewel-cafe.jp/wp-content/themes/jewelcafe_replace/assets/images/icon/mic_icon.png" alt="" />
		</div>
		!-->
		...
		<div class="search-btn">
			...
			<input type="image" id="form_search_btn" src="https://jewel-cafe.jp/wp-content/themes/jewelcafe_replace/assets/images/icon/icon_search.svg">
...
<div class="title">
	<div class="only-sp">
		<div class="search-shop-en search-shop-flex -jsstart">
			<div class="index sopia bold">または、お住いの地域からも検索できます</div>
...
var recognition = new webkitSpeechRecognition();
  ...
  if (event.results.length > 0) {
    ...
    skeyword.value = event.results[0][0].transcript;
		  ...
		  if (data.results) {
			...
			$.ajax({
				...
				success: function(response){
					...
					$("html,body").animate({scrollTop:$('#shop-res2').offset().top-120},300);
