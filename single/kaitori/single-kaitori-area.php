	...
	<div id="page-top"></div>
			<div class="main-section">
				<div class="only-pc">
					...
					<img src="<?php echo esc_url($image_fv_pc['url']);?>" alt="<?php echo $post->post_title;?>金券買取ならジュエルカフェへ" >
					<?php endif;?>
				</div>
				<div class="only-sp">
					<?php if(!empty($image_fv_sp)):?>
					<img src="<?php echo esc_url($image_fv_sp['url']);?>" alt="<?php echo $post->post_title;?>金券の買取ならジュエルカフェへ" >
					<?php endif;?>
				</div>
			</div>
...
?>		
			...
			<?php if(isset($parent_terms_slug) && $parent_terms_slug === 'diamond'): // 共通?>
	...
	?>
			...
			<div class="section-inner">
				...
				<section class="shop-area-city new">
				...
				<?php while($the_query->have_posts()): $the_query->the_post();?>
						...
						<li class="shop-area-city-item">
							<div class="container">
								<div class="tel">
									<a href="tel:<?php the_field('店舗電話番号'); ?>">TEL <?php the_field('店舗電話番号'); ?></a>
								</div>
								<div class="pic">
								
									<img src="<?php echo get_field('slide_img_1')['sizes']['large'];?>" alt="">
								</div>
								<div class="details_link">
									<a href="<?php the_permalink( );?>">
										<span class="sp">詳しくはこちら</span>
										<span class="pc">店舗詳細ページへ</span>
									</a>
								</div>
								<div class="name_address">
									<div class="name"><?php the_title();?></div>
									<div class="address"><?php the_field('所在地');?></div>
								</div>
								<div class="time"><span>営業時間</span><?php the_field('営業時間');?></div>
							</div>
						</li>
...
<script> //営業時間の※以降のテキストを削除する
  $(document).ready(function() {
    $('.shop-area-city.new').find('.time').each(function() {
      var $time = $(this);
      $time.contents().each(function() {
...
<?php */ ?>
			...
			<?php if ($purchase_title && !is_single('junk-rolex')):?>
			...
			<?php
				...
				if ( $post->post_type =='kaitori'):
					...
					//子ページ
					if( isset($parent_terms_slug) && $post->post_parent > 0  && $parent_terms_slug == 'tokei'){


							$template_file = 'template-parts/tokei-one-price-accordion';

					}else if( isset($parent_terms_slug) &&  $post->post_parent > 0  && $parent_terms_slug == 'brand' ){
					...
					}else if( isset($parent_terms_slug) &&  $post->post_parent > 0  && $parent_terms_slug == 'card'){
			...
			<section class="kaitori-resuluts">
				...
				<div class="section-inner">
					...
					<?php
						...
						if(isset($area_terms)&& is_array($area_terms)){
...
$terms_area = get_the_terms($post->ID,'area');
            ...
            <div class="text_box sp">
              ...
              <label class="read_more" for="trigger<?php echo $count; ?>"></label>
					...
					<?php
						if ( isset($news) && $news->post_count > 0 ) {
					?>
...
?>
		...
		<?php
			...
			if($kaitori_area_parent_id > 0 || $post->post_name  == 'letter'||$post->post_name == 'card'){
