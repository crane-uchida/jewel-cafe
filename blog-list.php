...
$blog_name = $wp->query_vars['blog'];


$cate_args = array(
	'tax_query' => array(
		array(
				'taxonomy' => 'hinmoku', //カスタムタクソノミー
				'field' => 'slug',
				'terms' => $blog_name, //タクソノミーターム
			)
		)
);
...
<?php get_header();?>
		...
		<section class="breadcrumbs_type2">
			...
			<div class="breadcrumbs">
				<div class="section-inner">
					...
					<a href="<?php echo esc_url( home_url('/') ) ?>">最新相場で高価買取ならジュエルカフェ<span></span></a>
					...
					<?php
					if(isset($hash_tag[$blog_name])){

						if($blog_name== 'k18'){

						echo '<a class="tag" href="/kaitori/gold">';
						echo '金・貴金属買取<span></span></a>';

						}else{

						echo '<a class="tag" href="/kaitori'.$hash_tag[$blog_name]['url'].'">';
						echo $hash_tag[$blog_name]['name'].'買取<span></span></a>';
					...
					<span>
						...
						if(isset($hash_tag[$blog_name])){
							...
							echo $hash_tag[$blog_name]['name'].'買取の参考価格・買取実績一覧';
    ...
    <div class="section-inner">
			<h1 class="section-ja-title">
				<?php

						if(isset($hash_tag[$blog_name])){

							echo $hash_tag[$blog_name]['name'].'の参考価格・買取実績一覧';
			...
			<?php get_template_part( 'template-parts/search-blog',null,$args = array('blog_name' => $blog_name));?>
			...
			if(!isset($tax_query_arr)){$tax_query_arr='';}
				...
				$args = array(
					...
					'orderby' =>  'DESC',
					'paged' => $paged,
					'tax_query' => array($tax_query_arr,$cate_args)
				);
